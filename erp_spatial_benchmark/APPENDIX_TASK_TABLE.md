...
| `absolute_direction_mc` | `spherical_localization_and_panoramic_topology` | `In the complete 360 panorama, which direction sector best contains {target_ref}?` or `Which absolute panorama sector best matches {target_ref} in the ERP image?` | Convert target yaw to one of 8 absolute sectors: `front`, `front-right`, `right`, `back-right`, `back`, `back-left`, `left`, `front-left`. Distractors come from neighboring and opposite sectors. | Direction tasks reject large targets (`x_fov > 35°`, `y_fov > 30°`, or `area_ratio > 0.08`). They also require effective boundary clearance after accounting for BFOV extent. `absolute_direction_mc` currently requires effective sector margin `>= 15°`. | Exact match on correct option key or option text. |
