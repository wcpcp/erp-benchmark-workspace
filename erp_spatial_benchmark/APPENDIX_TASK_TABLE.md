...
| `seam_continuity_mc` | `erp_representation_understanding` | `When the left and right ERP edges are understood as connected, which listed object is actually nearest in panoramic direction to {target_ref}?` or `If the ERP panorama is treated as one continuous 360 ring rather than a flat rectangle, which candidate lies closest to {target_ref}?` | The correct answer is a specific object reference from the image. Builder chooses a boundary-touching target and finds the opposite-boundary candidate with the smallest wrap-around yaw gap. The answer is therefore content-grounded, not a generic topology slogan. Metadata records `correct_ref`, `correct_wrap_gap_deg`, `correct_flat_x_gap_px`, and the flat-image lure fields. | Target must touch a true ERP boundary (`seam_contact_side`). The correct candidate must touch the opposite boundary, have wrap gap `<= 15°`, and still appear far in flat x-distance (`flat_x_gap >= 0.65 * erp_width`). At least one distractor must be a **flat-image lure** with small flat x-distance (`<= 0.25 * erp_width`) but much worse wrap gap (`>= max(35°, correct_gap + 20°)`). Two more distractors must satisfy the larger-gap condition. During final public selection, seam items are pooled under one task and sampled in stable random order. | Exact match on correct option key or option text. Always included in manual review. |
...
|---|---|
| `seam_continuity_mc` | Whether the model correctly reasons that the true panoramic nearest neighbor can lie across the left-right ERP boundary, even when the flat rectangle suggests a more tempting but wrong nearby distractor |
