...
| `absolute_direction_mc` | `spherical_localization_and_panoramic_topology` | `In the complete 360 panorama, which direction sector best contains {target_ref}?` or `Which absolute panorama sector best matches {target_ref} in the ERP image?` | Convert target yaw to one of 8 absolute sectors, but keep only rear-half challenge cases as correct answers: `back-right`, `back`, `back-left`. Distractors are chosen from more clearly separated sectors rather than immediate neighbors. When natural candidates are insufficient, derived yaw rotations may supplement these rear-sector cases. | Direction tasks reject large targets (`x_fov > 35°`, `y_fov > 30°`, or `area_ratio > 0.08`). They also require effective boundary clearance after accounting for BFOV extent. `absolute_direction_mc` currently requires effective sector margin `>= 15°` and excludes immediate neighboring distractor sectors. | Exact match on correct option key or option text. |
